...
def train(techarst, n_gram):
    #n_counts = defaultdict(Counter)
    counter = Counter()
    for i in range(0, len(techarst) - n_gram + 1):
        t = techarst[i:i + n_gram]
        #n_char = techarst[i + n_gram - 1]
        counter[t] += 1
    return counter
    ...
    with open(corpus_file,'r',encoding="utf8") as file:
        ...
        with open(model_file, 'w', encoding="utf8") as outputFile:
            ...
            for j in trigram:
                ...
                print(j, "\t",format("%.5f") % prob, file=outputFile,flush=False)
            ...
            for i in bigram:
                ...
                # print("chars=",i[0: 2],chars , "i=",i,trigram[i], "prob=",prob)
                print(i, "\t", format("%.5f") % prob, file=outputFile, flush=False)
